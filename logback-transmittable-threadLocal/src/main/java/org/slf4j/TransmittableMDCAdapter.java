 ...
 * <p/>
 * 重写MDCAdapter接口,将ThreadLocal替换成TransmittableThreadLocal对象
 * 作用于子父线程之间传递MDC数据丢失问题
 * <p/>
 *
 * @author Ceki G&uuml;lc&uuml;
 * @author: 苦瓜不苦
 * @date: 2022/6/28 23:23
 */
public class TransmittableMDCAdapter implements MDCAdapter {
    ...
    // 使用TransmittableThreadLocal类,父子线程可以相互传递数据
    ...
    private static final TransmittableMDCAdapter transmittableMDCAdapter;

    // 静态构造,创建新的对象,并且重新覆盖设置MDCAdapter对象,使其生效
    static {
        transmittableMDCAdapter = new TransmittableMDCAdapter();
        MDC.mdcAdapter = transmittableMDCAdapter;
    }

    // 提供外接创建实例对象
    public static MDCAdapter getInstance() {
        return transmittableMDCAdapter;
    }

    public TransmittableMDCAdapter() {
